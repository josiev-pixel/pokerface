...
append-only: an accepted ADR is never rewritten — it is superseded by a later ADR.
...
- **Only a single ARCHITECTURE.md** — loses the history and the "why" behind
  superseded choices.
- **Decisions in commit messages / chat only** — not discoverable later.
